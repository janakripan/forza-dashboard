    ...
    <div className="bg-white rounded-xl border border-[#E2E8F0] overflow-hidden shadow-sm">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
              ...
              <th className="px-6 py-4 text-[12px] font-bold text-[#64748B] uppercase tracking-wider">Item Details</th>
              <th className="px-6 py-4 text-[12px] font-bold text-[#64748B] uppercase tracking-wider">Category</th>
              <th className="px-6 py-4 text-[12px] font-bold text-[#64748B] uppercase tracking-wider">In-Hand Qty</th>
              <th className="px-6 py-4 text-[12px] font-bold text-[#64748B] uppercase tracking-wider">Unit Type</th>
              ...
              <tr key={index} className="hover:bg-[#F8FAFC] transition-colors group">
                ...
                <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                        <div className={`w-9 h-9 rounded bg-[#F1F5F9] border border-[#E2E8F0] flex items-center justify-center text-[12px] font-bold text-[#64748B] relative`}>
                            {item.code}
                            {item.status === 'low' && (
                        ...
                        <span className="text-[14px] font-bold text-[#1E293B]">{item.name}</span>
                    </div>
                </td>
                <td className="px-6 py-4">
                    <span className="text-[14px] font-bold text-[#1E293B] uppercase tracking-tight">{item.category}</span>
                </td>
                <td className="px-6 py-4 text-[15px] font-black text-[#1E293B]">{item.inHandQty}</td>
                <td className="px-6 py-4">
                    <span className="text-[14px] font-bold text-[#1E293B] uppercase tracking-tight">{item.unitType}</span>
                </td>
      ...
      <div className="px-6 py-4 bg-[#F8FAFC] border-t border-[#E2E8F0] flex flex-col sm:flex-row justify-between items-center gap-4">
        <p className="text-[14px] font-medium text-[#64748B]">
          Showing <span className="font-bold text-[#1E293B]">{startIndex + 1}</span> to <span className="font-bold text-[#1E293B]">{Math.min(startIndex + itemsPerPage, data.length)}</span> of <span className="font-bold text-[#1E293B]">{data.length.toLocaleString()}</span> results
        </p>
