    ...
    <div className="bg-white rounded-xl border border-[#E2E8F0] overflow-hidden shadow-sm">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-[#F1F5F9]">
            {currentData.map((item, index) => {
              const showSeparator =
                index > 0 && item.date !== currentData[index - 1].date;
              ...
              return (
                <React.Fragment key={index}>
                  ...
                  <tr className="hover:bg-[#F8FAFC] transition-colors group">
                    ...
                    <td className="px-6 py-4 text-[14px] font-semibold text-[#1E293B]">
                      {item.date}
                    </td>
                    <td className="px-6 py-4 text-[14px] font-bold text-[#5949BE] hover:underline cursor-pointer">
                      {item.invoiceNo}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded bg-[#E2E8F0] flex items-center justify-center text-[12px] font-bold text-[#64748B]">
                          AC
                        </div>
                    ...
                    <td className="px-6 py-4">
                      {item.type === "Split" ? (
                        ...
                        <span
                          className={`px-4 py-1.5 rounded-lg text-[13px] font-bold min-w-[100px] inline-block text-center ${getTypeStyle(item.type)}`}
                        >
                          {item.type}
                        </span>
                      )}
      ...
      <div className="px-6 py-4 bg-[#F8FAFC] border-t border-[#E2E8F0] flex flex-col sm:flex-row justify-between items-center gap-4">
        <p className="text-[14px] font-medium text-[#64748B]">
          Showing Page{" "}
          <span className="font-bold text-[#1E293B]">{currentPage}</span> of{" "}
          <span className="font-bold text-[#1E293B]">{totalPages}</span>
        </p>
